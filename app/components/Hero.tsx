    ...
    <section className="bg-[#111827] min-h-screen flex items-center relative overflow-hidden">
      ...
      <nav className="absolute top-0 left-0 right-0 z-20 px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <a href="mailto:adam@oneweeklabs.com?subject=API-Cert%20API%20Key%20Request" className="text-sm bg-[#D97706] text-white px-4 py-2 rounded-xl hover:bg-amber-600 transition-colors font-medium">Get Free Key</a>
        </div>
      </nav>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-24">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <a
                ...
                className="inline-flex items-center justify-center px-8 py-4 bg-[#D97706] text-white font-semibold rounded-xl hover:bg-amber-600 transition-colors duration-200 text-lg shadow-lg shadow-amber-900/20"
              >
                Get Free API Key
              </a>
              <a
                href="#api"
                className="inline-flex items-center justify-center px-8 py-4 border border-gray-600 text-gray-300 font-semibold rounded-xl hover:bg-gray-800 hover:border-gray-500 transition-colors duration-200 text-lg"
              >
                View Examples
              </a>
            </div>
          ...
          <div className="relative">
            <div className="bg-[#1F2937] rounded-2xl border border-gray-700/50 shadow-2xl overflow-hidden">
              {/* Terminal header */}
              <div className="flex items-center gap-2 px-4 py-3 bg-[#111827] border-b border-gray-700/50">
                <div className="w-3 h-3 rounded-full bg-red-500/80" />
                <div className="w-3 h-3 rounded-full bg-yellow-500/80" />
                <div className="w-3 h-3 rounded-full bg-green-500/80" />
                <span className="ml-3 text-gray-500 text-sm font-mono">api-cert response</span>
              </div>
              ...
              <div className="p-6 font-mono text-sm sm:text-base overflow-x-auto">
                <pre className="text-gray-400">
                  {displayedLines.filter((line): line is string => typeof line === 'string').map((line, index) => (
                    <div key={index} className="leading-relaxed">
                      ...
                      {line.includes('"oig_excluded": false') && (
                        <span className="bg-emerald-400/10 -mx-2 px-2 rounded">
                          {line.replace('false', '')}
                  ...
                  {isTyping && (
                    <span className="inline-block w-2 h-5 bg-gray-500 animate-pulse ml-1" />
                  )}
                </pre>
              </div>
            </div>
            ...
            {/* Latency badge */}
            <div className="absolute -bottom-4 -right-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-2">
              <span className="text-emerald-400 font-mono text-sm">6ms response</span>
            </div>
          </div>
        </div>
      </div>
